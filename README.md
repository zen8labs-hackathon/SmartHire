...
| `LLM_PROVIDER` | Which LLM backend the Next.js app uses. Default `vercel_gateway`. Supported values: `vercel_gateway`, `gemini` (direct Google AI Studio), `openai_compatible` / `litellm` (any OpenAI-compatible `/v1` proxy such as [LiteLLM](https://docs.litellm.ai/)). |
| `LLM_MODEL` | Optional **global** model id for the selected provider. For `vercel_gateway`, use [AI Gateway catalog ids](https://vercel.com/ai-gateway/models) (e.g. `openai/gpt-4o-mini`). For `gemini`, use Gemini ids (e.g. `gemini-2.0-flash`). For `openai_compatible`, use the model id exposed by your proxy. If unset: falls back to `AI_GATEWAY_JD_MATCH_MODEL`, then provider default (`openai/gpt-4o-mini` for gateway / OpenAI-compatible, `gemini-2.0-flash` for Gemini). |
| `LLM_BASE_URL` | OpenAI-compatible API base URL including `/v1` (e.g. `https://litellm-api.up.railway.app/v1`). Required when `LLM_PROVIDER=openai_compatible` or `litellm`. |
| `LLM_API_KEY` | API key for the OpenAI-compatible / LiteLLM proxy — **server only**. Required when `LLM_PROVIDER=openai_compatible` or `litellm`. |
| `AI_GATEWAY_API_KEY` | [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) key — **server only**. Powers JD document extraction and LLM-assisted **CV vs job-description match** when primary is `vercel_gateway`. Also used as **runtime fallback** when primary is LiteLLM/Gemini and the primary call fails (unless `LLM_FALLBACK_PROVIDER=off`). |
| `LLM_FALLBACK_MODEL` | Optional Gateway model id for fallback (default `openai/gpt-4o-mini`). Not the same as `LLM_MODEL` when that id is LiteLLM-specific. |
| `LLM_FALLBACK_PROVIDER` | Set to `off` / `none` / `false` to disable Vercel fallback. |
| `GOOGLE_GENERATIVE_AI_API_KEY` | Google AI Studio API key — **server only**. Required when `LLM_PROVIDER=gemini`. |
