...
| `jobs` | JD + metadata + S3 path + `criteria` (đã gộp job openings + descriptions) |
...
| `job_evaluate_templates`, `candidate_evaluation_reviews` | Template PDF + bản đánh giá + `preview_token` |
...
- Cột SSO trên `users` đã có; Microsoft SSO chưa implement
