...
export async function POST(request: NextRequest) {
  ...
  try {
    rows = await withTransaction(async (tx) => {
      const inserted = await insertManyFileUploads(tx, null, batchId, inputs);
      await createBatchDone(tx, batchId);
      return inserted;
    });
  } catch (e) {
    logApiError("Upload-files history (pool): insert failed", e, { batchId });
    return Response.json(
  ...
  return Response.json({ batchId, files: rows }, { status: 201 });
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logApiError("Upload-files history (pool): list failed", e, {});
    return Response.json(
