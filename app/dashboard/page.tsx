...
function FeatureLinkCard({ href, title, description, icon, locked = false, statusText }: FeatureCardProps) {
  const content = (
    <Card
      variant="secondary"
...
export default async function DashboardPage() {
  ...
  const displayName = profile?.username ?? user.email?.split("@")[0] ?? "Recruiter";
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="My Account Overview" description="Your workspace profile and credentials.">
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="flex items-center gap-3 rounded-xl border border-divider/50 bg-surface-secondary/20 p-3.5">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted">Email address</p>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-divider/50 bg-surface-secondary/20 p-3.5">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted">Access role</p>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-divider/50 bg-surface-secondary/20 p-3.5">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-bold uppercase tracking-wider text-muted">My Chapters</p>
              <div className="mt-0.5 flex flex-wrap gap-1">
                {chapterNames.length > 0 ? (
                  chapterNames.map((ch, idx) => (
                    <span key={idx} className="inline-flex items-center rounded bg-surface-tertiary px-1.5 py-0.5 text-[10px] text-foreground border border-divider">
                      {ch}
                    </span>
                  ))
                ) : (
                  <span className="text-xs text-muted font-medium truncate">None assigned</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <FeatureLinkCard
            ...
            description="Access the central talent pool, upload CVs, and monitor general pipeline states."
